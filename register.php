...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['dob']) || $_POST['dob'] === '') {
        ...
        } else if ($epoch > time() || $epoch < strtotime('-100 years', time())) {
            $validation['dob'] = 'Date of birth must be a valid date.';
        } else {
            $dob = $_POST['dob'];
        }
    ...
    if (count($validation) === 0) {
        ...
        $query = $pdo->prepare('INSERT INTO users(username, email, password, fname, lname, dob, sex, lang, license_accepted) VALUES(?, ?, ?, ?, ?, ?, ?, ?, 1)');
        $query->execute([$username, $email, $hash, $fname, $lname, $dob, $sexDbval, $_POST['lang']]);

        $user_id = $pdo->lastInsertId();
        $query2 = $pdo->prepare("SELECT * FROM users WHERE id = ?");
